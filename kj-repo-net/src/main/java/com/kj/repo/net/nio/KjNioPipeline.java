...
public class KjNioPipeline {

    public ByteBuffer read(SocketChannel channel, ByteBuffer buffer) throws IOException {
        return this.getHandlers().get(0).read(channel, buffer);
    }

    public ByteBuffer wrap(byte[] bytes) {
        List<KjNioHandler> handlers = this.getHandlers();
    ...
    public List<KjNioHandler> getHandlers() {
        return Lists.newArrayList(new KjNioHandler() {
            @Override
            public ByteBuffer read(SocketChannel channel, ByteBuffer buffer) throws IOException {
                if (buffer.limit() < 3) {
                    return buffer;
    ...
    public KjNioHandler next(KjNioHandler handler) {
        Iterator<KjNioHandler> iterator = this.getHandlers().iterator();
